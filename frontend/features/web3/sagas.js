...
const contractAddress = '0xE3DA70ae448CeA8D064698687C22E4DDDE17787e';
...
export function* initWeb3Saga() {
  ...
  if (window.ethereum) {
    ...
    try {
      ...
      const contract = new web3.eth
        .Contract(VerifyHash.abi, contractAddress)
  ...
  // Non-dapp browsers...
    ...
    yield put(initWeb3Error('Non-Ethereum browser detected. You should consider trying MetaMask!'))
  }
}
...
export function* sendVerifyHashSaga(action) {
  ...
  const { digest, hashFunction, size } = Utils.multiHashToBytes32(
    hash
  );
  try {
    const contract = yield select(getContract);
    const accounts = yield select(getAccounts);
    yield call(contract.methods.setEntry(digest, hashFunction, size).send, { from: accounts[0] });
...
export function* fetchEntriesSaga() {
    ...
    const [digestArray, hashFunctionArray, sizeArray] = [
      entries[0],
      entries[1],
      entries[2]
    ];
    const result = digestArray.map((digest, i) => (
      Utils.bytes32ToMultiHash({
